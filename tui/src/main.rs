    ...
    if !conf_path.exists() || panes.is_empty() {
        let entries = vec![StartupEntry {
            ...
            message: if !conf_path.exists() {
                "panes.conf not found -- run panebot-setup.sh to configure.".to_string()
            } else {
                "No panes defined -- run panebot-setup.sh to configure.".to_string()
            },
        }];
        ...
        loop {
            if let Event::Key(k) = event::read()? {
                match k.code {
                    KeyCode::Enter | KeyCode::Char('q') => return Ok(Vec::new()),
                    _ => {}
                }
            }
        }
    ...
    for i in 0..panes.len() {
        ...
        if alive {
            ...
            if let Some(last) = entries.iter_mut().rfind(|e| e.name == panes[i].name) {
                if started {
                    refresh_pane(&mut panes[i]);
                    last.status  = "Running".to_string();
                    last.message = "Loaded.".to_string();
                } else {
                    last.status  = "Offline".to_string();
                    last.message = "Could not start.".to_string();
                }
            }
            render_startup_screen(terminal, &entries, None, false)?;
            std::thread::sleep(Duration::from_millis(120));
