                ...
                Screen::Playlist(idx) => {
                    ...
                    if pl_add_input {
                        match key.code {
                            ...
                            KeyCode::BackTab => {
                                if !pl_completions.is_empty() {
                                    pl_comp_sel = if pl_comp_sel == 0 { pl_completions.len()-1 } else { pl_comp_sel-1 };
                                    pl_add_buf = pl_completions[pl_comp_sel].clone();
                                }
                            }
                            KeyCode::Backspace => { pl_add_buf.pop(); pl_completions = complete_path(&pl_add_buf); pl_comp_sel = 0; }
                            KeyCode::Char(c)   => { pl_add_buf.push(c); pl_completions = complete_path(&pl_add_buf); pl_comp_sel = 0; }
                            _ => {}
                        }
