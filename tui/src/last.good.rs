...
fn bootstrap_environment(layout_name: &str) -> io::Result<BootstrapResult> {
    ...
    if !conf.exists() {
        ...
        writeln!(f, "# geometry = 650x366+0+0   # used only if no slot is set")?;
    ...
    Ok(BootstrapResult { layout_name: layout_name.to_string() })
}
    ...
    terminal.draw(|f| {
        ...
        for line in boot_lines {
            items.push(ListItem::new(
                Line::from(Span::styled(line.clone(), Style::default().fg(C_HINT)))
            ).style(Style::default().bg(C_BG)));
        }
...
fn startup_sequence(
    ...
    let boot_lines = vec![
        format!("[PaneBot] :: Checking Environment :: Done!"),
        format!("[PaneBot] :: Loading Our Layout :: \" {} \"", boot.layout_name),
        format!("[PaneBot] :: Bringin' The Panes ::"),
    ];
    ...
    if !conf_path.exists() || panes.is_empty() {
        ...
        loop {
            if let Event::Key(k) = event::read()? {
                match k.code {
                    KeyCode::Enter | KeyCode::Char('q') => return Ok((Vec::new(), slots)),
                    _ => {}
                }
            }
        }
...
impl DashboardState {
    fn new() -> Self {
        DashboardState { cursor: 0, cmd_mode: false }
    }
}
...
fn main() -> Result<(), io::Error> {
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let comp_h     = if pl.add_input && !pl.completions.is_empty() { pl.completions.len() as u16 + 1 } else { 0 };
            ...
            match &screen {
                ...
                Screen::Playlist(idx) => {
                    ...
                    f.render_stateful_widget(List::new(items).style(Style::default().bg(C_BG)), chunks[3], &mut st);
                    if pl.add_input && !pl.completions.is_empty() {
                    ...
                    f.render_widget(divider_line(size.width as usize, div_color), chunks[5]);
                    f.render_widget(render_playlist_statusbar(pl.item_cmd, pl.move_input, &pl.move_buf, pl.add_input, &pl.add_buf), chunks[6]);
                }
            }
        })?;
                ...
                Screen::Dashboard => {
                    ...
                    if dash.cmd_mode {
                        ...
                        match key.code {
                            ...
                            KeyCode::Up   => { if dash.cursor > 0 { dash.cursor -= 1; } }
                            KeyCode::Down => { if dash.cursor < panes.len() - 1 { dash.cursor += 1; } }
                ...
                Screen::Playlist(idx) => {
                    ...
                    if pl.add_input {
                        ...
                        match key.code {
                            ...
                            KeyCode::Backspace => {
                                pl.add_buf.pop();
                                if !is_stream { pl.completions = complete_path(&pl.add_buf); pl.comp_sel = 0; }
                            }
                            KeyCode::Char(c) => {
                                pl.add_buf.push(c);
                                if !is_stream { pl.completions = complete_path(&pl.add_buf); pl.comp_sel = 0; }
                        ...
                        match key.code {
                            KeyCode::Up   => { if pl.cursor > 0 { pl.cursor -= 1; } }
                            KeyCode::Down => { if pl.cursor < pl.items.len().saturating_sub(1) { pl.cursor += 1; } }
