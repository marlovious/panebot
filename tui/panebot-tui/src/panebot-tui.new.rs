...
async fn resolve_daemon_addr(terminal: &mut Term) -> io::Result<Option<String>> {
    ...
    if hosts.is_empty() {
        return Ok(Some(LOCAL_ADDR.to_string()));
    }

    // Always offer localhost as first option
    hosts.insert(0, Host {
...
async fn handle_key(
    ...
    if app.show_details {
        ...
        // Back to dashboard — handled before match to avoid guard issues
