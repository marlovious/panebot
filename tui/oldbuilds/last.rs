...
fn duplicate_pane_to_conf(src: &Pane, new_name: &str) -> io::Result<()> {
    let conf = config_dir().join("panes.conf");
    let mut content = std::fs::read_to_string(&conf)?;
    let n = new_name.to_lowercase();
    let new_socket   = format!("~/.config/panebot/{}/{}.sock", n, n);
    let new_playlist = format!("~/.config/panebot/{}/{}.m3u", n, n);
    let mut entry = format!("\n[{}]\n", new_name);
    entry.push_str(&format!("socket   = {}\n", new_socket));
    entry.push_str(&format!("type     = {}\n", src.pane_type));
    entry.push_str(&format!("playlist = {}\n", new_playlist));
...
fn create_pane_files(name: &str, pane_type: &str) -> io::Result<()> {
    ...
    if !mpv_conf.exists() {
        ...
        writeln!(f)?;
        match pane_type {
            "audio" => { writeln!(f, "vid=no")?; }
            "ytdlp" => { writeln!(f, "ytdl-format=bestvideo+bestaudio")?; }
            "rtsp"  => {
            ...
            "http"  => { writeln!(f, "# ytdl=yes          # uncomment if using yt-dlp for HTTP streams")?; }
            _       => { writeln!(f, "# VIDEO pane — no special defaults")?; }
        }
    }
...
// Standard header: [PaneBot] :: {msg_spans} ::
// All screens use this — never construct the frame manually.
fn panebot_header(msg_spans: Vec<Span<'static>>) -> Paragraph<'static> {
    let mut spans = vec![
...
struct StartupEntry { name: String, status: String }
...
fn show_boot_status(
    ...
    terminal.draw(|f| {
        ...
        let prompt_h: u16 = if let Some(p) = prompt { if p.browsing { 2 } else { 1 } } else { 0 };
        ...
        let header = if complete {
            panebot_header(vec![
                Span::styled("Startup Complete", Style::default().fg(C_HINT)),
                Span::styled(" :: ", Style::default().fg(C_ORANGE)),
                Span::styled("[Enter]", Style::default().fg(C_CYAN)),
            ])
        } else {
            // Parse "label :: value" or just "label"
            let parts: Vec<&str> = msg.splitn(2, " :: ").collect();
            if parts.len() == 2 {
                panebot_header(vec![
                    Span::styled(parts[0].to_string(), Style::default().fg(C_HINT)),
                    Span::styled(" :: ", Style::default().fg(C_ORANGE)),
                    Span::styled(parts[1].to_string(), Style::default().fg(C_CYAN)),
                ])
            } else {
                panebot_header(vec![
                    Span::styled(msg.clone(), Style::default().fg(C_HINT)),
                ])
            }
        };
        ...
        if let Some(p) = prompt {
            ...
            opt_spans.push(Span::styled("  ::  ", Style::default().fg(C_ORANGE)));
            ...
            f.render_widget(Paragraph::new(lines).style(Style::default().bg(C_CMD_BG)), chunks[4]);
        }
    })?;
    Ok(())
}
...
fn startup_sequence(
    ...
    // Boot status messages shown one at a time in the header with a pause
    let boot_msgs: Vec<String> = {
        let mut v = vec!["Checking Environment :: Done!".to_string()];
        if boot.conf_was_missing {
            v.push("No Panes Found! :: Creating Default Layout".to_string());
        }
        v.push(format!("Assigning Slots Layout :: {}", boot.layout_name));
        v.push("Bringin' The Panes".to_string());
        v
    };

    for msg in &boot_msgs {
        show_boot_status(terminal, msg, &entries, None, false)?;
        let pause = if msg.contains("Bringin'") { 900 } else { 600 };
    ...
    for i in 0..panes.len() {
        ...
        if alive {
            ...
            entries.push(StartupEntry { name: panes[i].name.clone(), status: "Active".to_string() });
            show_boot_status(terminal, "Bringin' The Panes", &entries, None, false)?;
            ...
            entries.push(StartupEntry { name: panes[i].name.clone(), status: "Offline".to_string() });

            let mut prompt = PromptState {
                has_stored, has_default,
                browsing: false, browse_buf: String::new(),
                completions: Vec::new(), comp_sel: 0,
            };

            let choice = loop {
                show_boot_status(terminal, "Bringin' The Panes", &entries, Some(&prompt), false)?;
    ...
    show_boot_status(terminal, "Startup Complete", &entries, None, true)?;
    loop {
        if let Event::Key(k) = event::read()? {
            match k.code {
...
fn render_layout_capture(lc: &LayoutCaptureState) -> (Paragraph<'static>, List<'static>, Paragraph<'static>) {
    ...
    let header = match &lc.step {
        ...
        LayoutCaptureStep::Done => panebot_header(vec![
            ...
            Span::styled(" :: ", Style::default().fg(C_ORANGE)),
            Span::styled("[Enter]", Style::default().fg(C_CYAN)),
            Span::styled(" to return.", Style::default().fg(C_HINT)),
        ]),
        _ => panebot_header(vec![
    ...
    match &lc.step {
        ...
        LayoutCaptureStep::Done | LayoutCaptureStep::AssigningPanes(_) => {}
    }
...
fn render_slot_assign(lc: &LayoutCaptureState) -> (Paragraph<'static>, List<'static>, Paragraph<'static>) {
    ...
    let statusbar = Paragraph::new(Line::from(vec![
        Span::styled("[Tab]",   Style::default().fg(C_CMD_KEY)), Span::styled(" Cycle  ",   Style::default().fg(C_CMD_HNT)),
        Span::styled("[S-Tab]", Style::default().fg(C_CMD_KEY)), Span::styled(" Back  ",    Style::default().fg(C_CMD_HNT)),
        Span::styled("[Enter]", Style::default().fg(C_CMD_KEY)), Span::styled(" Confirm  ", Style::default().fg(C_CMD_HNT)),
        Span::styled("[Esc]",   Style::default().fg(C_CMD_KEY)), Span::styled(" Cancel",    Style::default().fg(C_CMD_HNT)),
    ])).style(Style::default().bg(C_CMD_BG));
...
fn render_dashboard_statusbar<'a>(cmd_mode: bool, layout_pick: bool) -> Paragraph<'a> {
    let spans = if layout_pick {
        Line::from(vec![
            Span::styled("[Tab]",   Style::default().fg(C_CYAN)), Span::styled(" Cycle:",    Style::default().fg(C_HINT)),
            Span::styled("[S-Tab]", Style::default().fg(C_CYAN)), Span::styled(" Back:",     Style::default().fg(C_HINT)),
        ...
        Line::from(vec![
            ...
            Span::styled("[D]",     Style::default().fg(C_CYAN)), Span::styled(" Duplicate:", Style::default().fg(C_HINT)),
...
impl DashboardState {
    fn new() -> Self {
        DashboardState {
            ...
            pending_layout: None,
            dup_input: false, dup_buf: String::new(), dup_src: String::new(),
        }
    }
}
...
fn main() -> Result<(), io::Error> {
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let comp_h     = if dash.layout_pick && !dash.layout_pick_list.is_empty() { 2 }
            ...
            match &screen {
                Screen::CaptureReady => {
                    f.render_widget(
                        panebot_header(vec![
                            Span::styled("New ", Style::default().fg(C_HINT)),
                            Span::styled("[n]", Style::default().fg(C_CYAN)),
                            Span::styled(" or Modify ", Style::default().fg(C_HINT)),
                            Span::styled("[m]", Style::default().fg(C_CYAN)),
                            Span::styled(" existing layout", Style::default().fg(C_HINT)),
                        ]),
                        chunks[1],
                    );
                ...
                Screen::Dashboard => {
                    ...
                    let header = if dash.dup_input {
                        Paragraph::new(Line::from(vec![
                            Span::styled("[PaneBot]", Style::default().fg(C_ORANGE)),
                            Span::styled(" :: ", Style::default().fg(C_ORANGE)),
                            Span::styled("Duplicated Pane config from ", Style::default().fg(C_HINT)),
                            Span::styled(dash.dup_src.to_uppercase(), Style::default().fg(C_CYAN)),
                            Span::styled(". Name this Pane: ", Style::default().fg(C_HINT)),
                            Span::styled(format!("{}_", dash.dup_buf), Style::default().fg(Color::White)),
                            Span::styled(" :: ", Style::default().fg(C_ORANGE)),
                        ])).style(Style::default())
                    } else {
                        render_dashboard_header(active, panes.len(), &layout_name)
                    };
                    f.render_widget(header, chunks[1]);
                    f.render_widget(divider_line(size.width as usize, C_DIVIDER), chunks[2]);
                    ...
                    if dash.layout_pick && !dash.layout_pick_list.is_empty() {
                        if let Some(name) = dash.layout_pick_list.get(dash.layout_pick_sel) {
                            let pick_items = vec![
                                blank_line(),
                                ListItem::new(Line::from(vec![
                                    Span::styled(">> ", Style::default().fg(C_ORANGE)),
                                    Span::styled(name.clone(), Style::default().fg(Color::White)),
                                ])).style(Style::default().bg(C_CURSOR)),
                            ];
                            f.render_widget(List::new(pick_items).style(Style::default()), chunks[4]);
                        }
                    }
            ...
            match &screen {
                ...
                Screen::Dashboard => {
                    ...
                    if dash.dup_input {
                        match key.code {
                            KeyCode::Enter => {
                                let new_name = dash.dup_buf.trim().to_string();
                                if !new_name.is_empty() {
                                    let src_idx = dash.cursor;
                                    let pane_type = panes[src_idx].pane_type.clone();
                                    let src = &panes[src_idx];
                                    let _ = duplicate_pane_to_conf(src, &new_name);
                                    let _ = create_pane_files(&new_name, &pane_type);
                                    panes = load_panes();
                                    for p in panes.iter_mut() { refresh_pane(p); }
                                }
                                dash.dup_input = false;
                                dash.dup_buf.clear();
                            }
                            KeyCode::Esc => { dash.dup_input = false; dash.dup_buf.clear(); }
                            KeyCode::Backspace => { dash.dup_buf.pop(); }
                            KeyCode::Char(c) => { dash.dup_buf.push(c); }
                            _ => {}
                        }
                    } else if dash.cmd_mode {
                        ...
                        match key.code {
                            ...
                            KeyCode::Tab => {
                                let max = dash.layout_pick_list.len().saturating_sub(1);
                                dash.layout_pick_sel = if dash.layout_pick_sel >= max { 0 } else { dash.layout_pick_sel + 1 };
                            }
                            KeyCode::BackTab => {
                                let max = dash.layout_pick_list.len().saturating_sub(1);
                                dash.layout_pick_sel = if dash.layout_pick_sel == 0 { max } else { dash.layout_pick_sel - 1 };
                            }
                            KeyCode::Enter => {
                        ...
                        match key.code {
                            ...
                            KeyCode::Char('L') => { screen = Screen::CaptureReady; }
                            KeyCode::Char('D') => {
                                dash.dup_input = true;
                                dash.dup_buf.clear();
                                dash.dup_src = panes[dash.cursor].name.clone();
                            }
                            KeyCode::Enter => {
                                pl.reset();
                                ...
                                LayoutCaptureStep::ModifyPick => {
                                    ...
                                    if let Some(name) = layouts.get(lc.modify_sel) {
                                        ...
                                        let _ = terminal.draw(|f| {
                                            let size = f.size();
                                            let chunks = Layout::default().direction(Direction::Vertical)
                                                .constraints([Constraint::Length(1),Constraint::Length(1),Constraint::Length(1),Constraint::Min(1),Constraint::Length(0),Constraint::Length(1)])
                                                .split(size);
                                            f.render_widget(Block::default(), size);
                                            f.render_widget(panebot_header(vec![
                                                Span::styled("Capturing geometry", Style::default().fg(C_HINT)),
                                            ]), chunks[1]);
                                            f.render_widget(divider_line(size.width as usize, C_ORANGE), chunks[2]);
                                        });
                    ...
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            if matches!(lc.step, LayoutCaptureStep::Done) {
                                ...
                                let next = cur + 1;
                                lc.step = if next < lc.captured.len() {
